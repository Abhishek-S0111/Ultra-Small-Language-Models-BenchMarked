# Helper functions for help throughout the process
...
def model_name_to_filename(name:str) -> str:
    """Convert the model name to a safe filename for easy storing of the model results on the various prompts"""
...
def find_model(name:str) -> str:
    """Look up a model in our available list of total models. Exit with error if not found."""
...
def save_model_results(run_dir:str, model_info: dict, runs_data: list[list[dict]], num_runs:int):
    """Write the model results into a file"""
...
def aggregate_runs(runs_for_model: list[list[dict]], num_runs: int) -> list[dict]:
    """Majority-vote aggregations across run for each model. returns one result per prompt"""
    aggregated = []
    ...
    for pi in range(num_prompts):
        ...
        #Propagate all tool calls: union valid tools appearing in > 50% of runs
